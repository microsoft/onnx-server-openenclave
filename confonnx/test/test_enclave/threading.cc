...
#include "test_t.h"
#include "test/test_enclave/threading.h"

// Threading code from https://github.com/microsoft/openenclave/blob/master/tests/libcxx/enc/enc.cpp.
const uint64_t MAX_ENC_KEYS = 4;
    ...
    void* arg)
{
    *enc_thread = 0;
    ...
    // enclave and host id
    if (OE_OK != test_host_create_thread(enc_key, oe_get_enclave()))
    {
...
static int _pthread_join_hook(pthread_t enc_thread, void**)
{
    ...
    int join_ret = 0;
    if (test_host_join_thread(&join_ret, join_enc_key) != OE_OK)
    {
...
static int _pthread_detach_hook(pthread_t enc_thread)
{
    ...
    int det_ret = 0;
    if (test_host_detach_thread(&det_ret, det_enc_key) != OE_OK)
    {
        printf("_pthread_detach_hook(): Error in call to host test_host_detach_thread for enc_key=%ld\n", det_enc_key);
        oe_abort();
    }
...
void TestEnclaveThreadFun(uint64_t enc_key)
{
